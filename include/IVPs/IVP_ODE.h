...
protected:
  ...
  int nnz; //number of non-zero elements in the Jacobian for the stiff terms
  int nnz_feval; //number of non-zero elements in the Jacobian of the full rhs
...
//***************************************************
  ...
  //***************************************************
  // Get the number of nonzero elements in the Jacobian matrix
  // of the stiff terms in the IVP_ODE
  inline int get_nnz() {return nnz;}
  //***************************************************                                              

  //***************************************************
  // Get the number of nonzero elements in the Jacobian matrix 
  // of the full RHS of the ODE system feval
  inline int get_nnz_feval() {return nnz_feval;}
  //***************************************************                                              
  ...
  //***************************************************
  //vector system function for the full stiff terms: G 
  // DY=G(t,Y)=F1(t,Y)+F2(t,Y)
  //***************************************************
  inline void G(const double t, const double *Y, double *DY)
  { double DY_tmp[neqn];
    F1 (t, Y, DY);
    F2 (t, Y, DY_tmp);
    cblas_daxpy(neqn,1.0,DY_tmp,1,DY,1);
...
///******************************************************
// Compute matrix  A=I-a*Jf, where Jf= Forward Difference Approximation of
// Jacobian of the function feval defining full rhs of the IVP_ODE
//******************************************************       
void Compute_feval_Matrix_FD(const double t, const double a, double *Y, LIS_MATRIX As)
  { 
    ...
    cout<<"............ neqn="<<neqn<<"    nonzeros="<<nonzeros<<endl;
    delete[] DY0;
    delete[] DYF;    
}
