...
//***************************************************
  ...
  void SBDF_Solver::init_CSR_LIS_matrix(LIS_MATRIX *A) {
...
//******************************************************
  void SBDF_Solver::compute_matrices(const double t, const double h, double ** Y) {
    ...
    //IVP->Compute_Matrix_FD(t, coef_idx,coef_idx, Y[idx], As);

    //IVP->Compute_Matrix_FD(t, coef_idx, coef_idx, Y[idx], As2);
    IVP->Compute_Matrix_Exact(t, coef_idx, coef_idx, Y[idx], As);
...
void  SBDF_Solver::compute_RHS0_SBDF(const double t, const double * h_vector, double** Y)
{
    ...
    if (order==1){
      // SBDF1: Compute R(Y1)=h*F3(t,Y[0])+Y[0]
      Add_aFY(t, h, Y[0]);
    }
    else {
      // R=alpha_{-1}*Y[0]
...
//***************************************************
void SBDF_Solver::Adaptive_dt_Integrate(const double t0, const double tf,
